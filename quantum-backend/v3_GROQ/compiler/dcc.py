...
def compile_compositional_ast(ir_dict: dict, force_solver = None) -> str:
    """
    V8 compiler implementation: parses ir_dict to symbolic CMM, 
    expands to OptimizationModel (OM) with reformulations, 
    and compiles deterministically to solver code.
    """
    ...
    try:
        ...
        solver_type = str(force_solver or "OR-Tools").upper()
        if "CQM" in solver_type or "DWAVE" in solver_type or "QUBO" in solver_type:
            return compile_om_to_cqm(om)
        else:
            return compile_om_to_ortools(om)
    except Exception as e:
        print(f"[DCC] CMM pipeline error: {e}. Falling back to legacy compile_v66.")
