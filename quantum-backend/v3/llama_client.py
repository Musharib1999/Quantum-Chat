...
async def call_adapter(
    ...
    """
    Call a LoRA adapter.
    For this un-adaptered Blackwell serving setup, it directly routes to
    the RunPod Qwen-32B model, parsing Llama prompt blocks.
    """
    from .execution_logger import log_engagement
    from .qwen_client import call_qwen
    
    system_ctx = f"Adapter Module Mapped to Qwen: {adapter_name}"
    
    # Parse prompt to system and user contents
    ...
    # Call Qwen directly on RunPod (without fallback to Groq)
    res = await call_qwen(
    ...
    log_engagement(f"Qwen-32B (Mapped from Adapter: {adapter_name})", system_ctx, prompt, res)
    return res
