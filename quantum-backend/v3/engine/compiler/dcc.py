...
"""
    ...
    @classmethod
    async def async_verify(cls, code: str, ir) -> dict:
        """
        Async version of verify(). Runs the sandboxed subprocess without blocking
        the FastAPI event loop. Safe for use inside async route handlers and agents.
        Delegates to a thread pool so the event loop remains free during subprocess wait.
        """
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(None, cls.verify, code, ir)
