...
def test_logical_relation_parsing_and_compilation():
    spec = {
        "entities_count": 15,
        "variable_registry": [
            {
        ...
        "constraint_registry": [
            ...
            {
                "name": "dependency_l11_implies_l9",
                "family": "dependency",
                "logical_relation": {
                    "type": "implication",
                    "if_variables": ["L11"],
    ...
    cmm = parse_spec_to_cmm(spec)
    assert len(cmm.patterns) == 2
    assert len(cmm.constraints) == 2
    
    # Verify mutual exclusion constraint
    exclusion_c = next(c for c in cmm.constraints if c.name == "mutual_exclusion_l5_l7")
    assert exclusion_c.op == "<="
    assert exclusion_c.right.value == 1.0
    
    # Verify implication constraint
    implication_c = next(c for c in cmm.constraints if c.name == "dependency_l11_implies_l9")
    assert implication_c.op == "<="
