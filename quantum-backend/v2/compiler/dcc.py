...
def compile_v66(ir: OptimizationIR, force_solver = None) -> dict:
    ...
    # 3. Match solver
    # ── QUBO hard guard: DCC has no QUBO translator. ─────────────────────────
    # QUBO problems MUST be routed through AutoQuboCompiler before reaching here.
    # If QUBO reaches this point it means a code path bypassed the Supervisor
    # guard — raise immediately so the bug is visible rather than silently
    # producing CQM code labelled as QUBO.
    if force_solver == "QUBO":
        raise ValueError(
            "[DCC] QUBO solver is not supported by the DCC compiler. "
            "Route QUBO problems through AutoQuboCompiler (compiler/autoqubo.py). "
            "Do NOT pass force_solver='QUBO' to compile_v66 or compile_compositional_ast."
        )
...
def compile_compositional_ast(ir_dict: dict, force_solver=None) -> str:
    """
    Legacy compatibility: accepts old-style flat IR dict, converts to
    OptimizationIR, and compiles via V6.6 pipeline.

    NOTE: QUBO is NOT supported by this compiler. Passing force_solver='QUBO'
    will raise a ValueError. Route QUBO through AutoQuboCompiler instead.
    """
    # ── QUBO hard guard ────────────────────────────────────────────────────
    if force_solver == "QUBO":
        raise ValueError(
            "[DCC] compile_compositional_ast does not support QUBO. "
            "Route QUBO problems through AutoQuboCompiler (compiler/autoqubo.py)."
        )
