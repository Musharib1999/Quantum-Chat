...
export default function TheProblemNode({ id }: NodeProps) {
  const { setNodes, setEdges } = useReactFlow();
  const { problemDefinition } = usePipelineStore();
    ...
    <div className="glass rounded-2xl p-5 min-w-[240px] border border-primary/20 relative group">
      ...
      <div className="bg-background/40 p-3 rounded-xl border border-border/50 mb-4 min-h-[60px] max-h-[100px] overflow-hidden">
        <p className="text-xs text-muted-foreground line-clamp-3 leading-relaxed">
          {problemDefinition || 'Describe your optimization problem in the chat panel to begin.'}
        </p>
      </div>
      ...
      <button 
        ...
        disabled={!problemDefinition}
        className="w-full py-2 bg-secondary hover:bg-secondary/80 text-foreground text-xs font-semibold rounded-xl transition-all shadow-sm active:scale-95 disabled:opacity-50"
      >
        View
      </button>
