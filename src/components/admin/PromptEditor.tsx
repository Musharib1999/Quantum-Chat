...
export default function PromptEditor() {
    ...
    if (prompts.length === 0) {
        return (
            <div className="p-8 text-center rounded-lg border border-[rgb(27,176,206)]/30 text-[#0F172A] bg-white">
    ...
    return (
        <div className="border rounded-xl overflow-hidden shadow-sm bg-white border-[rgb(27,176,206)]/30">
            <div className="p-6 border-b flex justify-between items-center sm:flex-row flex-col gap-4 border-[rgb(27,176,206)]/20">
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto">
                    <select
                        className="text-sm rounded-lg border block w-full sm:w-64 p-2.5 outline-none bg-white border-[rgb(27,176,206)]/30 text-[#0F172A] focus:border-[rgb(27,176,206)]"
            ...
            <div className="flex flex-col md:flex-row h-full md:h-[600px] divide-y md:divide-y-0 md:divide-x divide-slate-100">
                {/* Left Panel: Editor */}
                <div className="flex-1 flex flex-col p-6 relative bg-white/50">
                    ...
                    <textarea
                        className="flex-1 w-full border rounded-lg p-4 font-mono text-sm leading-relaxed resize-none focus:outline-none focus:border-[rgb(27,176,206)] transition-colors scrollbar-thin bg-white border-[rgb(27,176,206)]/30 text-[#0F172A] scrollbar-thumb-slate-300"
                    ...
                    <div className="mt-4 flex justify-end">
                        <button
                            ...
                            className={`flex items-center gap-2 px-5 py-2.5 rounded text-sm font-medium transition-all ${currentTemplate === activePrompt?.template
                                    ? 'bg-white text-[#0F172A] cursor-not-allowed'
                ...
                <div className="w-full md:w-80 p-6 flex flex-col bg-white">
                    <h3 className="font-medium mb-2 flex items-center gap-2 text-[#0F172A]">
                        <FileText size={16} className="text-[#0F172A]" />
                        Variables Required
                    </h3>
                    <p className="text-xs mb-6 text-[#0F172A]">
                    ...
                    <div className="space-y-3 overflow-y-auto scrollbar-thin pr-2 scrollbar-thumb-slate-300">
                        {activePrompt?.availableTags.map(tag => (
                            <div key={tag} className="flex flex-col gap-1 p-3 border rounded bg-white border-[rgb(27,176,206)]/20">
                                <code className="text-[#0F172A] text-xs font-bold">{tag}</code>
                            </div>
                        ))}
                        {(!activePrompt?.availableTags || activePrompt.availableTags.length === 0) && (
                            <div className="text-sm italic text-[#0F172A]">No variables required for this prompt.</div>
                        )}
                    </div>

                    <div className="mt-8 pt-6 border-t border-[rgb(27,176,206)]/20">
                        <h4 className="text-sm font-medium mb-2 text-[#0F172A]">Category Info</h4>
                        <p className="text-xs leading-relaxed mb-1 font-mono text-[#0F172A]">ID: {activePrompt?.category}</p>
                        <p className="text-xs leading-relaxed text-[#0F172A]">{activePrompt?.description}</p>
                        <p className="text-[10px] mt-4 text-[#0F172A]">Last Updated: {activePrompt ? new Date(activePrompt.updatedAt).toLocaleString() : 'N/A'}</p>
                    </div>
                </div>
            </div>
        </div>
