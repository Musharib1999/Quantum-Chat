...
import { Send, User, StopCircle, BookOpen } from 'lucide-react';
...
export default function ArticleChat({ contextConfig, placeholder, onAnalysisTriggered }: ArticleChatProps) {
    const {
        messages,
        inputValue,
        setInputValue,
        ...
        handleScroll,
        setShouldAutoScroll
    } = useQuantumChat('article', contextConfig);

    const lastTriggeredUrlRef = useRef<string | null>(null);

    // --- Article Analysis Trigger ---
    useEffect(() => {
        const targetUrl = contextConfig?.articleUrl;
        const targetName = contextConfig?.articleTitle;

        if (targetUrl && targetUrl !== lastTriggeredUrlRef.current) {
            lastTriggeredUrlRef.current = targetUrl;
            const triggerMessage = `Provide a detailed summary and latest insights for the research article: ${targetName}.`;
